...
// const LOCAL_IP_ADDRESS = "192.168.1.240";
...
let socket = io.connect(`http://${LOCAL_IP_ADDRESS}` + ':8000', {secure: false});
...
window.addEventListener("load", () => {
  roomName = new URLSearchParams(window.location.search).get("id");
  socket.emit("joinRoom", roomName);
  divRoomConfig.classList.add("d-none");
  roomDiv.classList.remove("d-none");
});
